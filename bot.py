        ...
        try:
            ...
            for tx in data:
                for transfer in tx.get('nativeTransfers', []):
                    if transfer['toUserAccount'] == SOL_SAFETY and transfer['amount'] >= 500000000: # 0.5 SOL
                        return True
            return False
        except: return False
...
async def start(m: types.Message):
    ...
    kb.button(text="🛰️ COMMAND CENTER", url="https://ninja-dashboard.onrender.com")
    kb.adjust(1)
...
async def handle_verify(m: types.Message):
    ...
    if await verify_tx(sig):
        army = [{"pub": str(kp.pubkey()), "priv": kp.to_json()} for kp in [Keypair() for _ in range(12)]]
        save_user_data(m.from_user.id, json.dumps(army))
        wallet_list = "\n".join([f"`{w['pub']}`" for w in army])
        await wait.edit_text(f"✅ **PAYMENT VERIFIED.**\n\n🛡️ **YOUR UNIQUE ARMY:**\n{wallet_list}\n\nFund each with 0.1 SOL to start Green Candles.", parse_mode="Markdown")
        await bot.send_message(VIP_CHANNEL, f"💰 **REVENUE:** 0.5 SOL received! New army provisioned.")
    else:
        await wait.edit_text("❌ Payment not found.")

@dp.message()
async def auto_audit(m: types.Message):
    ca = m.text.strip()
